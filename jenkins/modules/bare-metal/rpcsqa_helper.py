...
def install_opencenter(chef_node, install_script, role):
    root_pass = razor_password(chef_node)
    print ""
    print ""
    print "*****************************************************"
...
def print_server_info(chef_node):
...
def set_network_interface(chef_node):
    if "role[qa-base]" in chef_node.run_list:
        chef_node.run_list = ["recipe[network-interfaces]"]
        chef_node.save()
        print "Running network interfaces for %s" % chef_node.name
      
        #Run chef client thrice
        run1 = run_chef_client(chef_node, logfile="/dev/null")
        run2 = run_chef_client(chef_node, logfile="/dev/null")
        run3 = run_chef_client(chef_node, logfile="/dev/null")
...
def build_controller(chef_node, ha=False, ha_num=0):
    # Check for ha
    if ha:
    ...
    def environment_has_controller(environment):
        ...
        for node in nodes:
            ...
            if any(x in chef_node.run_list for x in roles):
                ...
                return False
>>>>>>> 198d5a2392bc8a65aa810b96245e3ff18ebe1e53
