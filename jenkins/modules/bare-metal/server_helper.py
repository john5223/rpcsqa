...
def install_opencenter_vm(vm_ip, oc_server_ip, install_script, role, user,
                          password):
    command = "bash <(curl %s) --role=%s --ip=%s" % (install_script,
                                                     role,
                                                     oc_server_ip)
...
def install_server_vms(controller_node, opencenter_server_ip, chef_server_ip,
                       vm_bridge, vm_bridge_device):
    controller_ip = controller_node['ipaddress']
    root_pass = razor_password(controller_node)
