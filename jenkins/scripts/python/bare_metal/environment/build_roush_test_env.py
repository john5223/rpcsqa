...
if active_models == {}:
    ...
    if results.display_only == 'false' and len(servers) > 0:
        ...
        commands=["apt-get install git python-pip -y", "git clone %s" % results.roush_test_repo, "pip install -r /root/roush-testerator/tools/pip-requires", "cat env.sh", "source env.sh; nosetests /root/roush-testerator/tests/test_happy_path.py"]
